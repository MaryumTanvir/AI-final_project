...
class CarState(object):
    ...
    def get_all_state_data(self):
        return {
            ...
            'z': self.z,
            **{f'track_{i}': val for i, val in enumerate(self.track) if self.track is not None},
            **{f'wheel_{i}': val for i, val in enumerate(self.wheelSpinVel) if self.wheelSpinVel is not None},
            **{f'opp_{i}': val for i, val in enumerate(self.opponents) if self.opponents is not None}
        }
